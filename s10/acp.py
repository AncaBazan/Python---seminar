...
class AnalizaComponentePrincipale():
    ...
    def creare_model(self, standardizare=True, nlib=0):
        # Pas 1: standardizarea setului de date => scadem din fiecare valoare media si impartim rezultatul la abaterea medie patratica
        if standardizare:
        ...
        # Pas 2: determinare matrice de covarianta
        r_cov = (1 / (n - nlib)) * x_.T @ x_

        # Pas 3: calcularea vectorilor si valorilor proprii (eigenvectors, eigenvalues)
        valp, vectp = np.linalg.eig(r_cov)

        # ordonare vectori proprii in ordinea semnificatiei valorilor proprii de la mare la mic
        ...
        # Calcul corelatii intre componentele principale si variabilele initiale
